...
def hero_picture(depth=0):
    ...
    Three separate crops rather than one image squeezed by object-fit:
    ...
    def srcset(stem, widths, ext):
        return ", ".join(f"{up}assets/img/{stem}-{w}.{ext} {w}w" for w in widths)

    import os as _os
    _here = _os.path.dirname(_os.path.abspath(__file__))
    def _have(stem, ws):
        return tuple(w for w in ws
                     if _os.path.exists(_os.path.join(_here, "assets/img", f"{stem}-{w}.avif")))
    wide = _have("hero-wide", (768, 1280, 1920, 2560))
    mid = _have("hero-mid", (600, 1024, 1536))
    tall = _have("hero-tall", (390, 780, 1170))
    alt = ("Design studio overlooking a city skyline, with drawings, a scale model "
           "and building models on screen")
    return (
        '<picture>'
        f'<source media="(min-width:1024px)" type="image/avif" srcset="{srcset("hero-wide", wide, "avif")}" sizes="100vw" width="1920" height="1080">'
        f'<source media="(min-width:1024px)" type="image/webp" srcset="{srcset("hero-wide", wide, "webp")}" sizes="100vw" width="1920" height="1080">'
        f'<source media="(min-width:600px)"  type="image/avif" srcset="{srcset("hero-mid", mid, "avif")}" sizes="100vw" width="1536" height="1152">'
        f'<source media="(min-width:600px)"  type="image/webp" srcset="{srcset("hero-mid", mid, "webp")}" sizes="100vw" width="1536" height="1152">'
        f'<source type="image/avif" srcset="{srcset("hero-tall", tall, "avif")}" sizes="100vw" width="1170" height="1560">'
        f'<source type="image/webp" srcset="{srcset("hero-tall", tall, "webp")}" sizes="100vw" width="1170" height="1560">'
        f'<img src="{up}assets/img/hero-wide-1280.webp" alt="{alt}" '
        f'width="1920" height="1080" loading="eager" fetchpriority="high" decoding="async">'
        '</picture>')
