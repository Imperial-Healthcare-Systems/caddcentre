...
from build_pages3 import (page_first_job_pakka, page_careers, page_life,
                          page_insights, render_article, render_legal, ARTICLES)
...
PAGES = [
    ...
    ("life", "life-at-cadd/index.html", 1,
     ...
     "life", page_life),
    ("insights", "insights/index.html", 1,
     "Insights — Engineering Careers & CAD Guides | CADD Centre Gurugram",
     "Straight answers about engineering careers, the software the industry uses, and what employers in Gurgaon are hiring for.",
...
for a in ARTICLES:
    PAGES.append((f"article:{a[0]}", f"insights/{a[0]}/index.html", 2,
                  f"{a[1]} | CADD Centre Gurugram", a[2], "insights", _mk(render_article, a[0])))
...
def build_mpa():
    ...
    for pid, path, depth, title, desc, active, fn in PAGES:
        html = (S.head(title, desc, "mpa", depth) + S.header("mpa", depth, active)
...
<a class="btn btn--ghost" href="/">Home</a></div>
    ...
    urls = "".join(
        ...
        f"<changefreq>monthly</changefreq><priority>{'1.0' if pid=='home' else '0.8'}</priority></url>\n"
